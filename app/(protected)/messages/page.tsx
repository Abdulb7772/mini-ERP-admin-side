...
export default function MessagesPage() {
  ...
  useEffect(() => {
    if (session?.user?.accessToken && !initializedRef.current) {
      ...
      const handleNewMessage = (data: { chatId: string; message: Message }) => {
        ...
        // Update chat list only if message is from someone else
      ...
      const handleChatUpdated = (data: { chatId: string }) => {
        ...
        if (data?.chatId && data.chatId === selectedChatRef.current?._id) {
          fetchMessagesForChat(data.chatId);
        }
      };
  ...
  const fetchChats = async () => {
    try {
      // Store current selected chat ID before clearing
      const currentSelectedChatId = selectedChatRef.current?._id;
      ...
      const fetchedChats = response.data?.data || response.data || [];
      setChats(fetchedChats);
      
      // If there was a selected chat, try to restore it with updated data
      if (currentSelectedChatId) {
        const updatedSelectedChat = fetchedChats.find((c: Chat) => c._id === currentSelectedChatId);
  ...
  const fetchMessagesForChat = async (chatId: string, shouldMarkAsRead = false) => {
    try {
      ...
      console.error("Error loading chat messages:", JSON.stringify(error, null, 2));
  ...
  const formatChatListTime = (chat: Chat) => {
    const timeValue =
      (typeof chat.lastMessage === "object" ? chat.lastMessage?.createdAt : undefined) ||
      chat.lastMessageAt;
  ...
  useEffect(() => {
    ...
    if (activeTab === "internal") {
      ...
      // Clear staffMembers and teams when not in those tabs
      setStaffMembers([]);
      setTeams([]);
    }
  }, [activeTab]);
  ...
  const handleSelectChat = async (chat: any) => {
    ...
    try {
      ...
      await fetchMessagesForChat(chat._id, true);
      setChats((prev) =>
        prev.map((item) =>
          item._id === chat._id
  ...
  return (
    <div className="h-[calc(100vh-4rem)] min-h-0 flex flex-col overflow-hidden bg-linear-to-br from-white via-gray-50 to-blue-50">
      {/* Header */}
      <div className="flex items-center justify-between p-3 sm:p-5 bg-linear-to-r from-blue-600 via-purple-600 to-indigo-600 shadow-lg shrink-0">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg sm:text-2xl font-bold text-white">Messages</h1>
        </div>
      </div>

      <div className="flex flex-1 min-h-0 overflow-hidden">
        {/* Chat List */}
        <div className={`${selectedChat ? 'hidden md:flex' : 'flex'} w-full md:w-1/3 border-r border-gray-200 flex-col min-h-0 bg-linear-to-b from-white to-gray-50`}>
          {/* Tabs */}
                ...
                {(activeTab === "internal" || activeTab === "teams" ? allUsers : chats).map((chat) => (
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-600 truncate ml-15 mt-2">
                      {(typeof chat.lastMessage === 'object' ? chat.lastMessage?.message : chat.lastMessage) || (chat.isPotential || chat.isPotentialTeam ? "✨ Start a conversation" : "No messages yet")}
                    </p>
        ...
        {/* Chat Area */}
        <div className={`${selectedChat ? 'flex' : 'hidden md:flex'} flex-1 min-h-0 flex-col overflow-hidden bg-white`}>
          {selectedChat ? (
            <>
              {/* Chat Header */}
              <div className="p-5 border-b border-gray-200 bg-linear-to-r from-gray-50 to-white">
                <div className="flex items-center gap-3">
                  ...
                  <div className="w-12 h-12 rounded-full bg-linear-to-br from-blue-500 via-purple-500 to-pink-600 flex items-center justify-center text-white font-bold text-lg shadow-lg shrink-0">
              ...
              {/* Messages */}
              <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden p-4 sm:p-6 space-y-4 bg-linear-to-b from-gray-50 to-white">
                {loading ? (
                    ...
                    {messages.map((message) => (
                      ...
                      >
                        <div className="relative group">
                          ...
                          >
                            {message.senderId !== session?.user?.id && (
                              <p className="text-xs font-semibold mb-1 break-word">{message.senderName}</p>
                            )}
                            {message.text && (
                              <p className="text-sm break-word mb-1">{message.text}</p>
                            )}
                            {message.contextType && message.contextId && (
                              <div 
                                className={`${message.text ? 'mt-2' : ''} mb-2 p-3 rounded-lg border cursor-pointer hover:opacity-80 transition ${
                                  message.senderId === session?.user?.id
                                    ? "bg-purple-400/40 border-purple-300/50"
                              ...
                              >
                                <div className="flex items-start gap-2">
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <p className="text-sm font-semibold mb-0.5">Attached {message.contextType}</p>
              ...
              {/* Message Input */}
              <div className="p-5 border-t border-gray-200 bg-linear-to-r from-white to-gray-50">
                {contextType && (
                ...
                <div className="flex gap-2 items-end">
                  ...
                  <input
                    ...
                    className="flex-1 min-w-0 border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white shadow-sm text-gray-900 placeholder-gray-400"
                  />
                  <button
                    onClick={handleSendMessage}
                    disabled={!messageText.trim()}
                    className="bg-linear-to-r from-blue-600 to-purple-600 text-white rounded-lg px-8 py-3 hover:from-blue-700 hover:to-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 font-bold shadow-lg hover:shadow-xl hover:scale-105 shrink-0"
                  >
